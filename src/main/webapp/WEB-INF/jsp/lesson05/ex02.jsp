...
<body>
	...
	<c:choose>
		<c:when test="${weight < 60}">치킨먹자<br>
		</c:when>
		<c:when test="${weight < 70}">샐러드 먹자<br>
		</c:when>
		<c:otherwise>굶자<br>
		</c:otherwise>
	</c:choose>
	...
	val:${val2} current:${status.current} first:${status.first} last:${status.last} count:${status.count} index:${status.index}<br>
	</c:forEach>
